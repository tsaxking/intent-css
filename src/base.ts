...
export type BaseCssConfig<G extends Globals> = {
  override?: Partial<CSSStyleDeclaration>; // user-provided CSS per tag
  include?: (keyof HTMLElementTagNameMap)[]; // whitelist of tags to generate
  exclude?: (keyof HTMLElementTagNameMap)[]; // blacklist of tags to skip
  tokens?: G; // engine globals for CSS variables
  themeName?: string; // optional data-theme scoping
  prefix?: string; // optional CSS variable prefix
};
...
const prefix = config.prefix ? config.prefix + '-' : '';
  ...
  for (const [tag, styleDecl] of Object.entries(texts)) {
    if (config.include && !config.include.includes(tag as keyof HTMLElementTagNameMap)) continue;
    if (config.exclude && config.exclude.includes(tag as keyof HTMLElementTagNameMap)) continue;
